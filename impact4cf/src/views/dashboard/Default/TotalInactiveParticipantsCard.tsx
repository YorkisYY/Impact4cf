...
interface TotalInactiveParticipantsCardProps {
  isLoading: boolean;
...
export default function TotalInactiveParticipantsCard({ isLoading, totalInactiveUsers, dateRange, change }: TotalInactiveParticipantsCardProps) {
  ...
  // Determine color based on change value - note that for inactive users, an increase is bad
  const getChangeColor = () => {
    if (change > 0) return '#FFAE00'; //  Red for positive change
    if (change < 0) return '#1EFF00'; //  Green for negative change
      ...
      {isLoading ? (
        ...
        >
          <Box sx={{ 
            ...
            >
              Participants who have not recorded an ACT Session this week 
            </Typography>
          </Box>
        </MainCard>
      )}
