...
// project imports
import BreathPressureChartPerBreath from './BreathPressureChartPerBreath';
...
export default function BreathInfo({ 
  isLoading, 
  setInfo, 
  prescriptionInfo, 
  date,
  ...
  if (isLoading) {
    return (
      <MainCard title="Breath Summary" content={false}>
        <CardContent>
          <Typography>Loading breath summary...</Typography>
        </CardContent>
      </MainCard>
    );
  }
  ...
  return (
    <MainCard title={`${formatDate(date)} - Session ${setInfo.sessionIndex} - ${setInfo.displayId} - ${breathInfo.displayId} Summary`} content={false}>
      <CardContent>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2, flexWrap: 'wrap', mb: 2 }}>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Typography variant="body1">
                  <strong>Duration:</strong> {formatDuration(breathInfo.duration)} 
                </Typography>
              </Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <AirIcon sx={iconSX} />
                <Typography variant="body1">
                  <strong>Mean Pressure:</strong> {formatPressure(breathInfo.pressure)} cmH₂O
                </Typography>
              </Box>
            </Box>
